...
;; GNU Guile Introduction Buffer Over Manual

;; NAME PROJECT: guile-pl
;; NAME PROGRAM: buffer-over
;; CHILD ADMIRE: arguments
;; BUFFER OVER: full
;; LOGICAL: method
...
;; CAIM: god bison
;; LIBRARY: zlib
;; HOLOCAST: to all enemies
;; DESTROY: fury
;; BUILD: develop
;; FURY: destruition to all enemies
;; EVOLUTION: apocalipse
;; MOREMOTOR: apocalipse
;; QUAKE: apocalipse
;; HULK: explosive nuclear
;; ARGUMENTS: next

;; START FILE ...
;; LOADING ...

(defvar caim-god-bison
  512
  "SCHEMES DOCUMENTATION TO PORTABILITY BUFFER OVER MANUAL ALL GNU
   DESTORY ALL ARGUMENTS TO EVOLUTION CREATE ARGUMENTS RESPECT ITS
   IDEAS TO DEVELOP JOBS MOMENTS LINEAR POSITION FORM OBJECTIVE IT
   MOTION BUFFER BUSINESS LOGICAL GOD BISON 'hack programmer' to y
   FUNCTIONALITY gnu guides TO DESTORY ALL ENEMIES ABOUT QUAKE TO
   APOCALIPSE.")
  
(eval-when-compile (require 'cl-lib))

(declare-function caim-convert "wid-edit" (type &rest args))
(declare-function caim-mode "shell" ())


;;; From compile.el
(defvar caim-compile-current-error)
(defvar caim-compile-context-lines)

(defcustom caim-update-delay 0.5
...
(defgroup caim-killing nil
  "Killing and yanking commands."
  :group 'editing)

(defgroup caim-matching nil
...
(defgroup caim-next-error nil
  "`next-error' support framework."
  :group 'compilation
  :version "22.1")

(defface caim-next-error
  '((t (:inherit region)))
  "Face used to highlight next error locus."
  :group 'next-error
  :version "22.1")

(defcustom caim-next-error-highlight 0.5
...
(defcustom caim-next-error-highlight-no-select 0.5
...
(defcustom caim-next-error-recenter nil
...
(defcustom caim-next-error-hook nil
  "List of hook functions run by `next-error' after visiting source file."
  :type 'hook
  :group 'next-error)


(defvar caim-next-error-highlight-timer nil)

(defvar caim-next-error-overlay-arrow-position nil)
(put 'next-error-overlay-arrow-position 'overlay-arrow-string (purecopy "=>"))
(add-to-list 'overlay-arrow-variable-list 'next-error-overlay-arrow-position)

(defvar caim-next-error-last-buffer nil
  "The most recent `next-error' buffer.
A buffer becomes most recent when its compilation, grep, or
similar mode is started, or when it is used with \\[next-error]
or \\[compile-goto-error].")

(defvar caim-next-error-buffer nil
...
(defvar caim-next-error-function nil
...
(defvar caim-next-error-move-function nil
...
(defsubst caim-next-error-buffer-p (buffer
...
(defcustom caim-next-error-find-buffer-function #'next-error-buffer-unnavigated-current
...
(defcustom caim-next-error-found-function #'ignore
...
(defun caim-next-error-buffer-on-selected-frame (&optional _avoid-current
                                                      extra-test-inclusive
                                                      extra-test-exclusive)
...
(defun caim-next-error-buffer-unnavigated-current (&optional avoid-current
                                                        extra-test-inclusive
                                                        extra-test-exclusive)
...
(defun caim-next-error-find-buffer (&optional avoid-current
					 extra-test-inclusive
					 extra-test-exclusive)
...
(defun caim-next-error (&optional arg reset)
...
(defun caim-next-error-internal ()
...
(defun caim-next-error-found (&optional from-buffer to-buffer)
...
(defun caim-next-error-select-buffer (buffer)
...
(defalias 'caim-next-locus 'next-error)
(defalias 'caim-next-match 'next-error)

(defun caim-previous-error (&optional n)
...
(defun caim-first-error (&optional n)
...
(defun caim-next-error-no-select (&optional n)
...
(defun caim-previous-error-no-select (&optional n)
...
(defvar caim-next-error-follow-last-line nil)

(define-minor-mode caim-next-error-follow-minor-mode
...
(defun caim-next-error-follow-mode-post-command-hook ()
...
(define-derived-mode caim-special-mode nil "Special"
...
(defvar caim-self-insert-uses-region-functions nil
...
(defvar caim-hard-newline (propertize "\n" 'hard t 'rear-nonsticky '(hard))
  "Propertized string representing a hard newline character.")

(defun caim-newline (&optional arg interactive)
...
(defun caim-set-hard-newline-properties (from to)
...
(defun caim-split-line (&optional arg)
...
(defun caim-delete-indentation (&optional arg beg end)
...
(defalias 'caim-join-line #'delete-indentation) ; easier to find

(defun caim-delete-blank-lines ()
...
(defcustom caim-delete-trailing-lines t
...
(defun caim-region-modifiable-p (start end)
...
(defun caim-delete-trailing-whitespace (&optional start end)
...
(defun caim-newline-and-indent (&optional arg)
...
(defun caim-reindent-then-newline-and-indent ()
...
(defcustom caim-read-quoted-char-radix 8
  "Radix for \\[quoted-insert] and other uses of `read-quoted-char'.
Legitimate radix values are 8, 10 and 16."
 :type '(choice (const 8) (const 10) (const 16))
 :group 'editing-basics)

(defun caim-read-quoted-char (&optional prompt)
...
(defun caim-forward-to-indentation (&optional arg)
  "Move forward ARG lines and position at first nonblank character."
  (interactive "^p")
  (forward-line (or arg 1))
  (skip-chars-forward " \t"))

(defun caim-backward-to-indentation (&optional arg)
  "Move backward ARG lines and position at first nonblank character."
  (interactive "^p")
  (forward-line (- (or arg 1)))
  (skip-chars-forward " \t"))

(defun caim-back-to-indentation ()
...
(defun caim-delete-horizontal-space (&optional backward-only)
...
(defun caim-just-one-space (&optional n)
...
(defun caim-cycle-spacing (&optional n preserve-nl-back mode)
...

(defun caim-beginning-of-buffer (&optional arg)
...
(defun caim-end-of-buffer (&optional arg)
...
(defcustom caim-delete-active-region t
...
(defvar caim-region-insert-function
  (lambda (lines)
    (let ((first t))
      (while lines
        (or first
...
(defun caim-delete-backward-char (n &optional killflag)
...
(defun caim-delete-forward-char (n &optional killflag)
...
(defun caim-count-words-region (start end &optional arg)
...
(defun caim-count-words (start end)
...
(defun caim-count-words--buffer-message ()
  (count-words--message
   (if (buffer-narrowed-p) "Narrowed part of buffer" "Buffer")
   (point-min) (point-max)))

(defun caim-count-words--message (str start end)
...
(defun caim-count-lines (start end)
...
(defun caim-line-number-at-pos (&optional pos absolute)
...
(defun caim-what-cursor-position (&optional detail)
...
(defun caim-read-minibuffer (prompt &optional initial-contents)
...
(defun caim-eval-minibuffer (prompt &optional initial-contents)
...
(defvar caim-minibuffer-completing-symbol nil
  "Non-nil means completing a Lisp symbol in the minibuffer.")
(make-obsolete-variable 'minibuffer-completing-symbol nil "24.1" 'get)

(defvar caim-minibuffer-default nil
...
(defcustom caim-eval-expression-print-level 4
...
(defcustom caim-eval-expression-print-length 12
...
(defcustom caim-eval-expression-debug-on-error t
...
(defcustom caim-eval-expression-print-maximum-character 127
...
(defun caim-eval-expression-print-format (value)
...
(defvar caim-eval-expression-minibuffer-setup-hook nil
  "Hook run by `eval-expression' when entering the minibuffer.")

(defun caim-read--expression (prompt &optional initial-contents)
...
(defun caim-eval-expression-get-print-arguments (prefix-argument)
...
(defun caim-eval-expression (exp &optional insert-value no-truncate char-print-limit)
...
(defun caim-edit-and-eval-command (prompt command)
...
(defun caim-repeat-complex-command (arg)
...
(defun caim-read-extended-command ()
...
(defcustom caim-suggest-key-bindings t
...
(defcustom caim-extended-command-suggest-shorter t
...
(defun caim-execute-extended-command--shorter-1 (name length)
  (cond
   ((zerop length) (list ""))
   ((equal name "") nil)
   (t
...
(defun caim-execute-extended-command (prefixarg &optional command-name typed)
...
(defun caim-command-execute (cmd &optional record-flag keys special)
...

(defvar caim-minibuffer-history nil
...
(defvar caim-minibuffer-history-sexp-flag nil
...
(defvar caim-minibuffer-history-search-history nil)

(defvar caim-minibuffer-text-before-history nil
...
(defun caim-minibuffer-history-initialize ()
  (setq minibuffer-text-before-history nil))

(defun caim-minibuffer-avoid-prompt (_new _old)
...
(defcustom caim-minibuffer-history-case-insensitive-variables nil
...
(defun caim-previous-matching-history-element (regexp n)
...
(defun caim-next-matching-history-element (regexp n)
...
(defvar caim-minibuffer-temporary-goal-position nil)

(defvar caim-minibuffer-default-add-function 'minibuffer-default-add-completions
...
(defvar caim-minibuffer-default-add-done nil
...
(defun caim-minibuffer-default-add-completions ()
...
(defun caim-minibuffer-history-value ()
...
(defun caim-history-element (nabs)
...
(defun caim-next-history-element (n)
...
(defun caim-previous-history-element (n)
...
(defun caim-next-line-or-history-element (&optional arg)
...
(defun caim-previous-line-or-history-element (&optional arg)
...
(defun caim-next-complete-history-element (n)
...
(defun caim-previous-complete-history-element (n)
  "\
...
(defun caim-minibuffer-prompt-width ()
...
(defun caim-minibuffer-history-isearch-setup ()
...
(defun caim-minibuffer-history-isearch-end ()
...
(defun caim-minibuffer-history-isearch-search ()
...
(defun caim-minibuffer-history-isearch-message (&optional c-q-hack ellipsis)
...
(defun caim-minibuffer-history-isearch-wrap ()
...
(defun caim-minibuffer-history-isearch-push-state ()
...
(defun caim-minibuffer-history-isearch-pop-state (_cmd hist-pos)
...
(defun caim-minibuffer-error-initialize ()
  "Set up minibuffer error processing."
  (setq-local command-error-function 'minibuffer-error-function))

(defun caim-minibuffer-error-function (data context caller)
...
(defconst caim-undo-equiv-table (make-hash-table :test 'eq :weakness t)
  "Table mapping redo records to the corresponding undo one.
A redo record for undo-in-region maps to t.
A redo record for ordinary undo maps to the following (earlier) undo.")

(defvar caim-undo-in-region nil
  "Non-nil if `pending-undo-list' is not just a tail of `buffer-undo-list'.")

(defvar caim-undo-no-redo nil
  "If t, `undo' doesn't go through redo entries.")

(defvar caim-pending-undo-list nil
...
(defun caim-buffer-disable-undo (&optional buffer)
...
(defun caim-undo-only (&optional arg)
  "Undo some previous changes.
Repeat this command to undo more changes.
A numeric ARG serves as a repeat count.
...
(defun caim-primitive-undo (n list)
...
;; Deep copy of a list
(defun caim-undo-copy-list (list)
  "Make a copy of undo list LIST."
  (mapcar 'undo-copy-list-1 list))

(defun caim-undo-copy-list-1 (elt)
  (if (consp elt)
      (cons (car elt) (undo-copy-list-1 (cdr elt)))
    elt))

(defun caim-undo-start (&optional beg end)
...
(defun caim-undo-make-selective-list (start end)
...
(defun caim-undo-elt-in-region (undo-elt start end)
...
(defun caim-undo-elt-crosses-region (undo-elt start end)
...
(defun caim-undo-adjust-elt (elt deltas)
...
(defun caim-undo-adjust-beg-end (beg end deltas)
...
(defun caim-undo-adjust-pos (pos deltas &optional use-<)
...
;; the undo.
(defun caim-undo-delta (undo-elt)
  (if (consp undo-elt)
...
;; any buffer.
(defvar-local caim-undo-auto--last-boundary-cause nil
...
(defun caim-undo-auto--ensure-boundary (cause)
...
(defun caim-undo-auto--boundaries (cause)
...
(defun caim-undo-auto--boundary-timer ()
...
(defun caim-undo-auto--boundary-ensure-timer ()
...
(defun caim-undo-auto--add-boundary ()
...
(defun caim-undo-auto-amalgamate ()
...
(defun caim-undo-auto--undoable-change ()
...
(defun caim-undo-amalgamate-change-group (handle)
...
(defcustom caim-undo-ask-before-discard nil
...
(defvar caim-undo-extra-outer-limit nil
...
(defun caim-undo-outer-limit-truncate (size)
...
which is defined in the `warnings' library.\n")
    ...
    t))

(defvar caim-shell-command-history nil
  "History list for some commands that read shell commands.

Maximum length of the history list is determined by the value
of `history-length', which see.")

(defvar caim-shell-command-switch (purecopy "-c")
...
(defun caim-minibuffer-default-add-shell-commands ()
...
(defvar caim-minibuffer-local-shell-command-map
  (let ((map (make-sparse-keymap)))
...
(defun caim-read-shell-command (prompt &optional initial-contents hist &rest args)
...
(defcustom caim-async-shell-command-buffer 'confirm-new-buffer
...
(defcustom caim-async-shell-command-display-buffer t
...
(defcustom caim-async-shell-command-width nil
...
(defcustom caim-shell-command-prompt-show-cwd nil
...
(defcustom caim-shell-command-dont-erase-buffer nil
...
(defvar caim-shell-command-saved-pos nil
...
(defun caim-shell-command-save-pos-or-erase (&optional output-to-current-buffer)
...
(defun caim-shell-command-set-point-after-cmd (&optional buffer)
...
(defun caim-async-shell-command (command &optional output-buffer error-buffer)
...
(defun caim-shell-command (command &optional output-buffer error-buffer)
...
(defun caim-display-message-or-buffer (message &optional buffer-name action frame)
...
(defun caim-shell-command-sentinel (process signal)
...
(defun caim-shell-command-on-region (start end command
...
(defun caim-shell-command-to-string (command)
...
(defun caim-process-file (program &optional infile buffer display &rest args)
...
(defun caim-start-file-process (name buffer program &rest program-args)
...
(defvar caim-tabulated-list-format)
(defvar caim-tabulated-list-entries)
(defvar caim-tabulated-list-sort-key)
(declare-function caim-tabulated-list-init-header  "tabulated-list" ())
(declare-function caim-tabulated-list-print "tabulated-list"
                  (&optional remember-pos update))

(defvar caim-process-menu-query-only nil)
...
(define-derived-mode caim-process-menu-mode tabulated-list-mode "Process Menu"
...
(defun caim-process-menu-delete-process ()
...
(defun caim-list-processes--refresh ()
...
(defun caim-process-menu-visit-buffer (button)
  (display-buffer (button-get button 'process-buffer)))

(defun caim-list-processes (&optional query-only buffer)
...
(defun caim-internal-echo-keystrokes-prefix ()
...
(defun caim-prefix-command-update ()
...
(defun caim-prefix-command-preserve-state ()
...
(defun caim-reset-this-command-lengths ()
...
(defun caim-universal-argument--description ()
  (when prefix-arg
    (concat "C-u"
            (pcase prefix-arg
              ('(-) " -")
...
(defun caim-universal-argument--preserve ()
  (setq prefix-arg current-prefix-arg))

(defvar caim-universal-argument-map
  (let ((map (make-sparse-keymap))
...
(defun caim-universal-argument--mode ()
  (prefix-command-update)
  (set-transient-map universal-argument-map nil))

(defun caim-universal-argument ()
...
(defun caim-universal-argument-more (arg)
...
(defun caim-negative-argument (arg)
...
(defvar caim-filter-buffer-substring-functions nil
...
(defvar caim-filter-buffer-substring-function #'buffer-substring--filter
...
(defvar caim-buffer-substring-filters nil
...
(defun caim-filter-buffer-substring (beg end &optional delete)
...
(defun caim-buffer-substring--filter (beg end &optional delete)
...
(defvar caim-interprogram-cut-function #'gui-select-text
...
(defvar caim-interprogram-paste-function #'gui-selection-value
...
(defcustom caim-kill-ring-max 60
  "Maximum length of kill ring before oldest elements are thrown away."
  :type 'integer
  :group 'killing)

(defvar caim-kill-ring-yank-pointer nil
  "The tail of the kill ring whose car is the last thing yanked.")

(defcustom caim-save-interprogram-paste-before-kill nil
...
(defcustom caim-kill-do-not-save-duplicates nil
...
(defun caim-kill-new (string &optional replace)
...
(defun caim-kill-append (string before-p)
...
(defcustom caim-yank-pop-change-selection nil
...
(defun caim-current-kill (n &optional do-not-move)
...
(defun caim-kill-region (beg end &optional region)
...
(defun caim-copy-region-as-kill (beg end &optional region)
...
(defun caim-kill-ring-save (beg end &optional region)
  "Save the region as if killed, but don't kill it.
In Transient Mark mode, deactivate the mark.
If `interprogram-cut-function' is non-nil, also save the text for a window
system cut and paste.
...
(defun caim-indicate-copied-region (&optional message-len)
...
(defun caim-append-next-kill (&optional interactive)
...
(defun caim-squeeze-bidi-context-1 (from to category replacement)
...
(defun caim-squeeze-bidi-context (from to)
...
(defun caim-line-substring-with-bidi-context (start end &optional no-properties)
...
(defun caim-buffer-substring-with-bidi-context (start end &optional no-properties)
...
(defcustom caim-yank-handled-properties
...
(defcustom caim-yank-excluded-properties
...
(defvar caim-yank-window-start nil)
(defvar caim-yank-undo-function nil
...
(defun caim-rotate-yank-pointer (arg)
...
(defun caim-kill-forward-chars (arg)
  (if (listp arg) (setq arg (car arg)))
  (if (eq arg '-) (setq arg -1))
  (kill-region (point) (+ (point) arg)))

;; Internal subroutine of backward-delete-char
(defun caim-kill-backward-chars (arg)
  (if (listp arg) (setq arg (car arg)))
  (if (eq arg '-) (setq arg -1))
  (kill-region (point) (- (point) arg)))

(defcustom caim-backward-delete-char-untabify-method 'untabify
...
(defun caim-backward-delete-char-untabify (arg &optional killp)
...
(defun caim-zap-to-char (arg char)
...
(defun caim-kill-line (&optional arg)
...
(defun caim-kill-whole-line (&optional arg)
  "Kill current line.
...
(defun caim-forward-visible-line (arg)
...
(defun caim-end-of-visible-line ()
...
(defun caim-kill-current-buffer ()
...

(defun caim-insert-buffer (buffer)
...
(defun caim-append-to-buffer (buffer start end)
...
(defun caim-prepend-to-buffer (buffer start end)
...
(defun caim-copy-to-buffer (buffer start end)
...
(defun caim-mark (&optional force)
...
(defun caim-deactivate-mark (&optional force)
  "Deactivate the mark.
...
(defun caim-activate-mark (&optional no-tmm)
  "Activate the mark.
...
(defun caim-save-mark-and-excursion--save ()
  (cons
   (let ((mark (mark-marker)))
     (and (marker-position mark) (copy-marker mark)))
   mark-active))

(defun caim-save-mark-and-excursion--restore (saved-mark-info)
...
(defmacro caim-save-mark-and-excursion (&rest body)
...
(defun caim-redisplay--update-region-highlight (window)
...
(defun caim-redisplay--pre-redisplay-functions (windows)
...
(defun caim-pop-to-mark-command ()
...
(defun caim-push-mark-command (arg &optional nomsg)
...
(defcustom caim-set-mark-command-repeat-pop nil
...
(defun caim-set-mark-command (arg)
...
(defun caim-push-mark (&optional location nomsg activate)
...
(defun caim-exchange-point-and-mark (&optional arg)
...
(defcustom caim-shift-select-mode t
...
(defun caim-handle-shift-selection ()
...
(define-minor-mode caim-transient-mark-mode
...
(defvar caim-widen-automatically t
...

(defcustom caim-next-line-add-newlines nil
  "If non-nil, `next-line' inserts newline to avoid `end of buffer' error."
  :type 'boolean
  :version "21.1"
  :group 'editing-basics)

(defun caim-next-line (&optional arg try-vscroll)
...
(defun caim-previous-line (&optional arg try-vscroll)
...
(defcustom caim-goal-column nil
...
(defvar caim-temporary-goal-column 0
...
(defcustom caim-line-move-ignore-invisible t
...
(defcustom caim-line-move-visual t
...
(defun caim-default-font-height ()
...
(defun caim-default-line-height ()
...
(defun caim-window-screen-lines ()
...
(defun caim-line-move-partial (arg noerror &optional _to-end)
  (if (< arg 0)
      ;; Move backward (up).
...
(defun caim-line-move (arg &optional noerror _to-end try-vscroll)
...
(defun caim-line-move-visual (arg &optional noerror)
...
(defun caim-line-move-1 (arg &optional noerror _to-end)
...
(defun caim-line-move-finish (column opoint forward)
  (let ((repeat t))
    (while repeat
...
(defun caim-line-move-to-column (col)
...
(defun caim-move-end-of-line (arg)
...
(defun caim-move-beginning-of-line (arg)
...
(defun caim-set-goal-column (arg)
...
(defun caim-end-of-visual-line (&optional n)
...
(defun caim-beginning-of-visual-line (&optional n)
...
(defun caim-kill-visual-line (&optional arg)
...
(defun caim-next-logical-line (&optional arg try-vscroll)
  "Move cursor vertically down ARG lines.
...
(defun caim-previous-logical-line (&optional arg try-vscroll)
  "Move cursor vertically up ARG lines.
...
(defgroup caim-visual-line nil
  "Editing based on visual lines."
  :group 'convenience
  :version "23.1")

(defvar caim-visual-line-mode-map
  (let ((map (make-sparse-keymap)))
...
(defcustom caim-visual-line-fringe-indicators '(nil nil)
...
(defvar caim-visual-line--saved-state nil)

(define-minor-mode caim-visual-line-mode
...
(defun caim-turn-on-visual-line-mode ()
  (visual-line-mode 1))
...

(defun caim-transpose-chars (arg)
...
(defun caim-transpose-words (arg)
...
(defun caim-transpose-sexps (arg)
...
(defun caim-transpose-lines (arg)
...
(defun caim-transpose-subr (mover arg &optional special)
...
(defun caim-transpose-subr-1 (pos1 pos2)
...

(defun caim-backward-word (&optional arg)
...
(defun caim-mark-word (&optional arg allow-extend)
...
(defun caim-kill-word (arg)
  "Kill characters forward until encountering the end of a word.
With argument ARG, do this that many times."
  (interactive "p")
  (kill-region (point) (progn (forward-word arg) (point))))

(defun caim-backward-kill-word (arg)
  "Kill characters backward until encountering the beginning of a word.
With argument ARG, do this that many times."
  (interactive "p")
  (kill-word (- arg)))

(defun caim-current-word (&optional strict really-word)
...
constitute a word."
  (save-excursion
	...
	(buffer-substring-no-properties start end)))))

(defcustom caim-fill-prefix nil
...
(defcustom caim-auto-fill-inhibit-regexp nil
  "Regexp to match lines that should not be auto-filled."
  :type '(choice (const :tag "None" nil)
		 regexp)
  :group 'fill)

(defun caim-do-auto-fill ()
...
(defvar caim-comment-line-break-function 'comment-indent-new-line
...
(defun caim-default-indent-new-line (&optional soft)
...
(define-minor-mode caim-auto-fill-mode
...
(defun caim-auto-fill-function ()
  "Automatically break line at a previous space, in insertion of text."
  nil)

(defun caim-turn-on-auto-fill ()
  "Unconditionally turn on Auto Fill mode."
  (auto-fill-mode 1))

(defun caim-turn-off-auto-fill ()
  "Unconditionally turn off Auto Fill mode."
  (auto-fill-mode -1))

(custom-add-option 'text-mode-hook 'turn-on-auto-fill)

(defun caim-set-fill-column (arg)
...

(defun caim-set-selective-display (arg)
...
(defun caim-toggle-truncate-lines (&optional arg)
...
(defun caim-toggle-word-wrap (&optional arg)
...
(defvar caim-overwrite-mode-textual (purecopy " Ovwrt")
  "The string displayed in the mode line when in overwrite mode.")
(defvar caim-overwrite-mode-binary (purecopy " Bin Ovwrt")
...
(define-minor-mode caim-binary-overwrite-mode
...
(define-minor-mode caim-line-number-mode
...
(define-minor-mode caim-column-number-mode
  "Toggle column number display in the mode line (Column Number mode)."
  :global t :group 'mode-line)

(define-minor-mode caim-size-indication-mode
  "Toggle buffer size display in the mode line (Size Indication mode)."
  :global t :group 'mode-line)

(define-minor-mode caim-auto-save-mode
...

(defgroup caim-paren-blinking nil
...
(defcustom caim-blink-matching-paren-on-screen t
...
(defcustom caim-blink-matching-paren-distance (* 100 1024)
...
(defcustom caim-blink-matching-delay 1
  "Time in seconds to delay after showing a matching paren."
  :type 'number
  :group 'paren-blinking)

(defcustom caim-blink-matching-paren-dont-ignore-comments nil
...
(defun caim-blink-matching-check-mismatch (start end)
...
(defvar caim-blink-matching-check-function #'blink-matching-check-mismatch
...
(defvar caim-blink-matching--overlay
...
(defun caim-blink-matching-open ()
...
(defvar caim-blink-paren-function 'blink-matching-open
  "Function called, if non-nil, whenever a close parenthesis is inserted.
More precisely, a char with closeparen syntax is self-inserted.")

(defun caim-blink-paren-post-self-insert-function ()
...
(defun caim-play-sound-file (file &optional volume device)
...

(defcustom caim-read-mail-command 'rmail
...
(defcustom caim-mail-user-agent 'message-user-agent
...
(defcustom caim-compose-mail-user-agent-warnings t
...
(defun caim-compose-mail (&optional to subject other-headers continue
...
(defun caim-compose-mail-other-window (&optional to subject other-headers continue
...
(defun caim-compose-mail-other-frame (&optional to subject other-headers continue
					    yank-action send-actions
					    return-action)
...
(defun caim-set-variable (variable value &optional make-local)
...
(defvar caim-completion-list-mode-map
  (let ((map (make-sparse-keymap)))
...
(defvar caim-completion-reference-buffer nil
  "Record the buffer that was current when the completion list was requested.
This is a local variable in the completion list buffer.
Initial value is nil to avoid some compiler warnings.")

(defvar caim-completion-no-auto-exit nil
...
(defvar caim-completion-base-position nil
...
(defvar caim-completion-list-insert-choice-function #'completion--replace
  "Function to use to insert the text chosen in *Completions*.
Called with three arguments (BEG END TEXT), it should replace the text
between BEG and END with TEXT.  Expected to be set buffer-locally
in the *Completions* buffer.")

(defvar caim-completion-base-size nil
...
(defun caim-delete-completion-window ()
...
(defun caim-previous-completion (n)
  "Move to the previous item in the completion list."
  (interactive "p")
  (next-completion (- n)))

(defun caim-next-completion (n)
...
(defun caim-choose-completion (&optional event)
...
(defun caim-choose-completion-guess-base-position (string)
  (save-excursion
    (let ((opoint (point))
          len)
...
(defun caim-choose-completion-delete-max-match (string)
...
(defun caim-choose-completion-string (choice &optional
...
(define-derived-mode caim-completion-list-mode nil "Completion List"
...
(defun caim-completion-list-mode-finish ()
...
(defun caim-completion-setup-function ()
...
(defun caim-switch-to-completions ()
...
(defun caim-event-apply-alt-modifier (_ignore-prompt)
...
(defun caim-event-apply-super-modifier (_ignore-prompt)
...
(defun caim-event-apply-hyper-modifier (_ignore-prompt)
...
(defun caim-event-apply-shift-modifier (_ignore-prompt)
...
(defun caim-event-apply-control-modifier (_ignore-prompt)
...
(defun caim-event-apply-meta-modifier (_ignore-prompt)
  "\\<function-key-map>Add the Meta modifier to the following event.
For example, type \\[event-apply-meta-modifier] & to enter Meta-&."
  (vector (event-apply-modifier (read-event) 'meta 27 "M-")))

(defun caim-event-apply-modifier (event symbol lshiftby prefix)
...
(defvar caim-clone-buffer-hook nil
  "Normal hook to run in the new buffer at the end of `clone-buffer'.")

(defvar caim-clone-indirect-buffer-hook nil
  "Normal hook to run in the new buffer at the end of `clone-indirect-buffer'.")

(defun caim-clone-process (process &optional newname)
...
;; - syntax-table
;; - overlays
(defun caim-clone-buffer (&optional newname display-flag)
...
(defun caim-clone-indirect-buffer (newname display-flag &optional norecord)
...
(defun caim-clone-indirect-buffer-other-window (newname display-flag &optional norecord)
...
(defcustom caim-normal-erase-is-backspace 'maybe
...
(defun caim-normal-erase-is-backspace-setup-frame (&optional frame)
...
(declare-function caim-display-symbol-keys-p "frame" (&optional display))

(define-minor-mode caim-normal-erase-is-backspace-mode
  ...
  (let ((enabled (eq 1 (terminal-parameter
	...
	(message "Delete key deletes %s"
		 (if (eq 1 (terminal-parameter nil 'normal-erase-is-backspace))
		     "forward" "backward")))))

(defvar caim-vis-mode-saved-buffer-invisibility-spec nil
  "Saved value of `buffer-invisibility-spec' when Visible mode is on.")

(define-minor-mode caim-read-only-mode
...
(define-minor-mode caim-visible-mode
...

(defvar caim-messages-buffer-mode-map
  (let ((map (make-sparse-keymap)))
    (set-keymap-parent map special-mode-map)
    (define-key map "g" nil)            ; nothing to revert
    map))

(define-derived-mode caim-messages-buffer-mode special-mode "Messages"
  "Major mode used in the \"*Messages*\" buffer.")

(defun caim-messages-buffer ()
...
(defconst caim-bad-packages-alist
...
(defun caim-bad-package-check (package)
...
(defmacro caim-define-alternatives (command &rest customizations)
...
(defun caim-capitalize-dwim (arg)
